...
def get_detailpage(main_url):
    ...
    for url in area_url:
        ...
        #if url == 'http://zu.sz.fang.com/house-a016375/':# or url == 'http://zu.sz.fang.com/house-a016375/':
            low = 0
            high = 1000
            price_url = url + 'h33'
...
#标题 所在区 价格 位置 出租形式 户型 平方 层数 朝向 经纬度
    print(info_list)
...
def mysql(city,data):
    con = pymysql.connect(host='45.76.212.82', port=3306, user="soufangwang", passwd="yunshuokeji", db="soufang",charset='utf8')
    cur = con.cursor()
    #标题,区位,价格,位置,出租形式,户型,平方,层数,朝向,经纬度                                                                                              #标题 所在区 价格 位置 出租形式 户型 平方 层数 朝向 经纬度
